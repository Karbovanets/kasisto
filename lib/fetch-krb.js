...
export const requestPayment = (url, username, password, amount = null, address = "KRB") => {
  ...
  let cancel = () => {
    ...
    window.clearInterval(UnconfirmedHandle);
  }
  ...
  const onUnconfirmedTransaction = (pollingInterval = 5000) => new Promise((resolve, reject) => {
    const poll = () => {
      wallet
        .getUnconfirmedTransaction({ addresses: [_address] })
        .then(
          result => {
            const length = result.transactionHashes.length;
    ...
    UnconfirmedHandle = window.setInterval(poll, pollingInterval);
    poll()
  })

...
export default class SendPayment extends Component {
  ...
  render () {
    ...
    const { setTip, setStatus, cancelPayment, settings, payment: { address, status, integratedAddress, exchange, fiatCurrency, requestedAmount, receivedAmount, convertedAmount, tip, uri, rate } } = this.props;
    ...
    if (status === "unconfirm") {
      return <Fragment>
        <div className="o-app__content o-content">
          <h3 className="u-medium">status: {status}</h3>
          <table className="c-payment-summary u-margin-bottom-large">
            <tbody>
              ...
              <tr>
                <th scope="row">Tip</th>
                <td>
                  {new Big(tip || "0").times(rate).toFixed(2)}
                  &nbsp;
                    {fiatCurrency}
                </td>
              </tr>
              <tr>
                ...
                <td>
                  {new Big(requestedAmount)
                    .add(new Big(tip || "0").times(rate))
                    .toFixed(2)}
                  &nbsp;
                    {fiatCurrency}
                </td>
          ...
          <small className="u-margin-bottom-large">
            {new Big(convertedAmount)
              .add(new Big(tip || "0"))
              .toFixed(12)}{" "}
            KRB
            </small>
    ...
    } else if (receivedAmount != null && new Big(receivedAmount).gte(new Big(convertedAmount))) {
      return <Fragment>
            ...
            <table className="c-payment-summary u-margin-bottom-large">
              <tbody>
                ...
                <tr>
                  <th scope="row">Tip</th>
                  <td>
                    {new Big(receivedAmount)
                      .times(rate)
                      .minus(requestedAmount)
                      .toFixed(2)}
                    &nbsp;
                    {fiatCurrency}
                  </td>
                ...
                <tr>
                  <th scope="row">Total</th>
                  <td>
                    {new Big(receivedAmount).times(rate).toFixed(2)}
                    &nbsp;
                    {fiatCurrency}
                  </td>
                </tr>
      ...
      return <Fragment>
          <div className="o-app__content o-content">
            <h3 className="u-margin-bottom">Add a tip?</h3>

            <ul className="o-list-inline c-tips">
              {[0.1, 0.15, 0.2].map((tipRate, key) => (
                <li
                  key={key}
                  onClick={e => {
                ...
                >
                  <a
                    href="#"
                    className={`c-tips__tip ${
                      key === tipIndex ? "c-tips__tip--active" : "bar"
                    }`}
                  >
                    {tipRate * 100}%
                  </a>
